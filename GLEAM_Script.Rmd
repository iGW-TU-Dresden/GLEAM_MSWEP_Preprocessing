...
while (is.na(start_year) || is.na(end_year)) {
  ...
  if (end_year <= start_year || end_year > 2025) {
    print("Error: End year cannot be earlier than the start year or later than the year 2025. Please enter valid years.")
    end_year <- as.numeric(readline(prompt = "Enter the end year you are interested in: "))
  }
}
...
cat("\nPlease enter your GLEAM SFTP login information.\n")
cat("You can obtain it from https://www.gleam.eu/\n\n")

host <- readline(
  prompt = "SFTP Host (e.g. aether.ugent.be): "
)

host <- gsub("^sftp://", "", trimws(host))

port <- readline(
  prompt = "SFTP Port (e.g. 2225): "
)
...
for (year in start_year:end_year) {
  ...
  remote_path <- sprintf(
    "sftp://%s:%s@%s:%s/data/v4.3a/daily/%d/%s_%d_GLEAM_v4.3a.nc",
    username,
    password,
    host,
    port,
  ...
  local_path <- file.path(
    nc_files,
    sprintf("%s_%d_GLEAM_v4.3a.nc", variable, year)
  )
