...
The **Global Land Evaporation Amsterdam Model (GLEAM)** estimates the different components of land evaporation. The following script guides you in downloading and processing the **GLEAM v3.8a** global dataset available from 1980 (January 1st) to 2022 (December 31st).
...
while (is.na(start_year) || is.na(end_year)) {
  ...
  if (end_year <= start_year || end_year > 2022) {
    print("Error: End year cannot be earlier than the start year or later than the year 2022. Please enter valid years.")
    end_year <- as.numeric(readline(prompt = "Enter the end year you are interested in: "))
  }
}
...
sftp_changedir(tofolder = sprintf("data/v3.8a/daily/%s", start_year),
...
for (year in start_year:end_year) {
  ...
  sftp_download(file = sprintf("%s_%s_GLEAM_v3.8a.nc", variable, year), tofolder = nc_files)
}
```
